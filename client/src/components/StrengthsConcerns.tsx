...
export default function StrengthsConcerns({ strengths, concerns }: StrengthsConcernsProps) {
  return (
    <section className="rounded-2xl border border-white/10 bg-[#111827] p-6">
      <h2 className="text-sm font-semibold uppercase tracking-widest text-gray-400">Strengths & Concerns</h2>
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div>
          ...
          {strengths.length === 0 ? (
            ...
            <ul className="space-y-2">
              {strengths.map((s, i) => (
                <li key={i} className="flex items-start gap-2 rounded-lg bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300">
                  <svg className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-emerald-400" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div>
          ...
          {concerns.length === 0 ? (
            ...
            <ul className="space-y-2">
              {concerns.map((c, i) => (
                <li key={i} className="flex items-start gap-2 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-300">
